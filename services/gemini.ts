...
export const generateYearStory = async (year: string): Promise<string> => {
  try {
    const prompt = `
    ...
    const response = await ai.models.generateContent({
      ...
      contents: prompt,
      config: {
        thinkingConfig: { thinkingBudget: 0 }, // Disable thinking for faster narrative generation
        temperature: 0.7,
      }
    });
    ...
    throw new Error("Failed to time travel. The wormhole is unstable.");
  }
};
